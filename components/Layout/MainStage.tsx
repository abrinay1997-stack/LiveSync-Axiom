        ...
        <div className="h-16 bg-black border border-white/5 rounded-xl flex items-center px-6 gap-8 shrink-0 shadow-2xl overflow-x-auto custom-scrollbar animate-slide-up">
          <div className="flex flex-col gap-1 pr-8 border-r border-white/5 shrink-0">
            ...
            <div className="flex gap-1">
              {(['none', '1/3', '1/12', '1/48'] as const).map(s => (
                <button key={s} onClick={() => setSmoothing(s)} className={`px-3 py-1 rounded-md text-[9px] font-bold border transition-all ${config.smoothing === s ? 'bg-cyan-500 border-cyan-500 text-black shadow-[0_0_10px_rgba(6,182,212,0.3)]' : 'bg-white/5 border-white/5 text-slate-500 hover:text-white'}`}>{s.toUpperCase()}</button>
              ))}
            </div>
          </div>
          ...
          <div className="flex flex-col gap-1 shrink-0">
             ...
             <div className="flex items-center gap-3">
                <button onClick={analysis.runAutoDelay} disabled={!isStarted} className="flex items-center gap-2 px-5 py-2 rounded-lg bg-cyan-500/10 border border-cyan-500/30 text-[10px] font-black text-cyan-400 hover:bg-cyan-500 hover:text-black transition-all disabled:opacity-20"><Activity size={14} /> FIND DELAY</button>
                {analysis.delayData && (
                  <div className="flex items-center gap-3 px-4 py-2 bg-black border border-white/10 rounded-lg"><Clock size={12} className="text-cyan-400" /><span className="text-[11px] mono font-bold text-white">{analysis.delayData.ms.toFixed(2)}ms</span></div>
                )}
             </div>
          </div>

          <div className="ml-auto flex items-center gap-8 shrink-0 border-l border-white/5 pl-8">
             <button onClick={analysis.resetAnalysis} className="p-3 bg-white/5 border border-white/10 rounded-xl text-slate-500 hover:text-white transition-all"><RotateCcw size={16} /></button>
          </div>
        </div>
      )}
    </section>
