          ...
          <div className="flex items-center gap-6 pr-8 border-r border-white/5 shrink-0">
             <div className="flex flex-col gap-1">
                ...
                <div className="flex gap-3">
                   <div className={`flex items-center border rounded-lg px-3 py-1.5 gap-2 ${config.maxDb <= config.minDb + 10 ? 'bg-rose-500/10 border-rose-500/30' : 'bg-white/5 border-white/10'}`} title="Limite superior del grafico (dB) - debe ser mayor que minDb"><Maximize2 size={12} className="text-cyan-400"/><input type="number" value={config.maxDb} onChange={(e) => { const v = Math.max(config.minDb + 10, Math.max(-50, Math.min(20, parseInt(e.target.value) || 0))); updateConfig({ maxDb: v }); }} className="bg-transparent text-[11px] mono text-white font-bold w-12 outline-none text-center" /></div>
                   <div className={`flex items-center border rounded-lg px-3 py-1.5 gap-2 ${config.minDb >= config.maxDb - 10 ? 'bg-rose-500/10 border-rose-500/30' : 'bg-white/5 border-white/10'}`} title="Limite inferior del grafico (dB) - debe ser menor que maxDb"><Minimize2 size={12} className="text-rose-400"/><input type="number" value={config.minDb} onChange={(e) => { const v = Math.min(config.maxDb - 10, Math.max(-150, Math.min(-20, parseInt(e.target.value) || -110))); updateConfig({ minDb: v }); }} className="bg-transparent text-[11px] mono text-white font-bold w-12 outline-none text-center" /></div>
                </div>
             </div>
